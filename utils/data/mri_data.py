...
def fetch_dir(
    key: str, data_config_file: Union[str, Path, os.PathLike] = "directory.yaml"
) -> Path:
    """
...
#                a = (hf["kspace"].shape[2],hf["kspace"].shape[3], 1)
#                b = (hf["reconstruction_rss"].shape[1],hf["reconstruction_rss"].shape[2],1)
...
#                recon_size = b
                ...
                if "Leader" in str(fname):
                    recon_fname = Path(str(fname).replace('kspace_','image_'))
                else: recon_fname = Path(str(fname).replace('kspace_',''))
                enc_size = (hf["kspace"].shape[2],hf["kspace"].shape[3], 1)
                ismask = True
                padding_left = -1
                padding_right = -1
                num_slices = hf["kspace"].shape[0]
                with h5py.File(recon_fname, "r") as hf_:
                    recon_size = (hf_["image_label"].shape[1],hf_["image_label"].shape[2],1)
        metadata = {
    ...
    def __getitem__(self, i: int):
        ...
        with h5py.File(fname, "r") as hf:
            ...
            if metadata["ismask"] == True:
                if "Leader" in str(fname):
                    recon_fname = Path(str(fname).replace('kspace_','image_'))
                else: recon_fname = Path(str(fname).replace('kspace_',''))
                if "mask" in hf:
                    mask = np.asarray(hf["mask"])
                else: mask = np.where(kspace[0,0,:] ==0,0,1) 
                with h5py.File(recon_fname, "r") as hf_:
                    target = hf_["image_label"][dataslice]
                    attrs = dict(hf_.attrs)
            else:
