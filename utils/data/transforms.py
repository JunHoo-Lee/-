...
class UnetDataTransform:
    ...
    def __call__(
        ...
        image = utils.ifft2c(masked_kspace)
        ...
        # absolute value
        image = utils.complex_abs(image)

        image = utils.rss(image)
...
class VarNetDataTransform:
    ...
    def __call__(
        ...
        if attrs["ismask"] == True:
            ...
            mask[:, :, :acq_start] = 0
            mask[:, :, acq_end:] = 0
        if attrs["istrain"] is True and random.random() > 2: 
            target = torch.flip(target,[1])
            masked_kspace = torch.flip(masked_kspace,[1])
        return (
            masked_kspace,
            mask.byte(),
            target,
