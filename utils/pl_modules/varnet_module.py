...
from utils.models import VarNet
...
class VarNetModule(MriModule):
    ...
    def configure_optimizers(self):
        optim = torch.optim.Adam(
            self.parameters(), lr=0, weight_decay=self.weight_decay
        )
        scheduler = CosineAnnealingWarmupRestarts(optim,first_cycle_steps=200, cycle_mult=1.5, max_lr=self.lr * 10, min_lr=self.lr * 0.7, warmup_steps=50, gamma=0.6)
