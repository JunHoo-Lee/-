...
class MriModule(pl.LightningModule):
    ...
    def validation_step_end(self, val_logs):
        ...
        for i, fname in enumerate(val_logs["fname"]):
            ...
            mse_vals[fname][slice_num] = torch.tensor(
                distance.mse(target, output)
            ).view(1)
            target_norms[fname][slice_num] = torch.tensor(
                distance.mse(target, np.zeros_like(target))
            ).view(1)
            ssim_vals[fname][slice_num] = torch.tensor(
                distance.ssim(target[None, ...], output[None, ...], maxval=maxval)
            ).view(1)
    ...
    def test_epoch_end(self, test_logs):
        ...
        utils.save_reconstructions(outputs, save_path)
